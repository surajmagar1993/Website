...
export default function Header({ settings }: { settings?: SiteSettings }) {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu Overlay */}
      {isOpen && (
        <div className="lg:hidden fixed inset-0 top-[72px] bg-[var(--color-bg)] z-40 overflow-y-auto border-t border-[var(--color-glass-border)]">
          <nav className="flex flex-col p-4 sm:p-6 pb-20">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}

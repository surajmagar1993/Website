...
import { ArrowLeft, UserPlus, User, Building, CheckCircle, AlertCircle, Trash2, Edit2, Globe, Users } from "lucide-react";
...
export default function UsersManagementPage() {
  ...
  const handleSaveUser = async (submittedData: Record<string, string>) => {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-bold text-white tracking-tight">Manage Clients</h1>
            <p className="text-[var(--color-text-secondary)] mt-1 text-sm">Create and manage client accounts with brand assets</p>
          </div>
          <button 
            onClick={() => openModal()}
            className="flex items-center gap-2 px-5 py-2.5 bg-[var(--color-primary)] text-black font-bold rounded-xl hover:opacity-90 transition-all shadow-lg shadow-[var(--color-primary)]/20 whitespace-nowrap"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          <Card className="glass border-white/10 bg-black/20 text-white">
            ...
            <CardContent>
              ...
              <p className="text-xs text-gray-500 mt-1">Portal accounts</p>
            </CardContent>
          </Card>
          <Card className="glass border-white/10 bg-black/20 text-white">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-400">
                {profiles.filter(p => new Date(p.created_at) > new Date(new Date().getFullYear(), new Date().getMonth(), 1)).length}
              </div>
